...
This file performs the setup Direct Refinement.

Based on GPT3.5-turbo as a base model (create statement, rewrite)
"""
from key import test_key, miri_key
from info import principles, rewriteR, yesnoCR_turned
import openai
import numpy as np
import time
import sys
...
print("Truly direct refinement (Turbo only)")
...
for rep in range(1):
    ...
    for i in principle_i:  # Which principles to test
            ...
            system_prompt_rewrite = "You will revise parts of a statement, based on a revision request. Only rewrite parts of the statement to comply with the revision request, and leave the rest of the statement as it was. You may want to delete a paragraph, or add a new one."
            chat_log = [
                {"role": "system", "content": system_prompt_rewrite},
                {"role": "system", "content": "### INPUT STORY: " + input_story + "### STATEMENT: " + initial_statement},
