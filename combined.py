...
use_model = "tuned_vicuna" #"turbo3.5" # "alpaca7", "turbo3.5", "vicuna13", "tuned_vicuna"
...
if use_model == "alpaca7":
    from alpaca_part import evaluate as a_gen
    evaluate = a_gen
    shots = 0
elif use_model == "vicuna13":
    from vicuna_part import evaluate as v_gen
    evaluate = v_gen
    shots = 0

elif use_model == "tuned_vicuna":
    from tuned_vicuna import evaluate as vt_gen
    evaluate = vt_gen
    shots = 0
else:
    print("Using GPT.")
    ...
    for i in principle_i:  # Which principles to test
        ...
        while True:
            ...
            if use_model == "turbo3.5":
                ...
                resp = ask_gpt(chat_log, temp=0.5, topp=0.9)
                critique = resp["choices"][0]["message"]["content"]
            else:
                resp = evaluate(
                    instruction=yesnoCR[i] + yesnoWhy + diff,  # yesno_intro + yesnoCR[i] + yesno_end,
            ...
            if yesnoWhy == yesno_A and not yesno_AR:
                # Ask turbo why current statement does not satisfy this principle
            ...
            if yesnoWhy == yesno_A and not yesno_AR:
                ...
                system_prompt_rewrite = "You will revise parts of a statement, based on a revision request. Rewrite what is criticized, and keep the rest as it was. You may want to delete a paragraph, or add a new one."
                chat_log = [
                    {"role": "system", "content": system_prompt_rewrite},
                    {"role": "system", "content": "### INPUT STORY: " + input_story + "\n### STATEMENT: " + initial_statement},
                    {"role": "user", "content": "### REVISION REQUEST: " + rewriteR[i] + no_hall},
...
print(f"We did a total of {loops} loops.")
print(initial_statement)
